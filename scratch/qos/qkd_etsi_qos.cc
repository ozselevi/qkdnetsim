 ...
 *
 * Execute using ./waf --run scratch/qos/qkd_etsi_qos_full.cc --command-template="mpirun -np 4 %s"
 *
...
// Attribútumokhoz szükséges headerek (ezek az eredeti szkriptben implicit módon használtak)
//#include "ns3/uinteger.h"
//#include "ns3/datarate.h"

// Az új QKD Applikáció header-ének beillesztése
#include "qkd-app-qos.h"

using namespace ns3;

NS_LOG_COMPONENT_DEFINE ("QKD_ETSI_QOS");
...
// Megjegyzés: A nlohmann::json külső függőség, a fordítás csak akkor működik, ha ez telepítve van és a Waf tudja kezelni.
// Az ns-3 környezet nem tartalmazza alapértelmezetten, de az eredeti fájl használja.
// Feltételezzük, hogy a qkdnetsim környezetben elérhető.
...
CreateOutputForCPU(std::string outputStatsName)
{
    ...
    for (std::map<std::string, LinkDetails* >::iterator it = m_connectionPairs.begin(); it != m_connectionPairs.end(); ++it) {
        ...
        if(it->second->type == 0){
            ...
            avgSizeOfConsumedKeys = it->second->m_keyIDConsumedInBuffers.empty() ? 0 : avgSizeOfConsumedKeys/it->second->m_keyIDConsumedInBuffers.size();
            ...
            avgSizeOfGeneratedKeys = it->second->m_keyIDGeneratedInBuffers.empty() ? 0 : avgSizeOfGeneratedKeys/it->second->m_keyIDGeneratedInBuffers.size();
            ...
            // Ezen a ponton uint32_t-be konvertálva elveszhet az AVG pontossága, de az eredeti kód így csinálta.
            temp[7] = static_cast<uint32_t>(it->second->m_avgSizeOfGeneratedKeys); 
            temp[8] = static_cast<uint32_t>(it->second->m_avgSizeOfConsumedKeys);
            ...
            avgSizeOfConsumedKeys = it->second->m_keyIDConsumedByKMS.empty() ? 0 : avgSizeOfConsumedKeys/it->second->m_keyIDConsumedByKMS.size();
            ...
            temp[45] = static_cast<uint32_t>(it->second->m_keysAvgSizeConsumedKMSBits);
        } 
...
int main (int argc, char *argv[])
{
    ...
    //////////////////////////////////////
    //  QKD APP ETSI 014 -> QKDAppQoS HASZNÁLATA
    //////////////////////////////////////

    if(numberOfETSI014ApplicationLinks && systemId == systemID0) {
        std::cout << "\n*********\n*** ETSI 014 Configuration (using QKDAppQoS)\n*********\n";
    }
    ...
    {   
        ...
        //Set default values for applications created below (QKDApp014 attribute names inherited by QKDAppQoS)
        ...
        // **********************************************
        // * QKDApp014 HELYETT QKDAppQoS HASZNÁLATA
        // **********************************************
        Ptr<QKDAppQoS> appAlice = CreateObject<QKDAppQoS> ();  
        appAlice->SetAttribute("AESLifetime", UintegerValue(aesLifetime));  
        appAlice->SetAttribute("NumberOfKeyToFetchFromKMS", UintegerValue (numberOfKeyToFetchFromKMS));
        appAlice->SetAttribute("AuthenticationType", UintegerValue (authenticationType)); 
        appAlice->SetAttribute("EncryptionType", UintegerValue (encryptionType)); 
        appAlice->SetAttribute("WaitInsufficient", TimeValue (Seconds (appHoldTime)));
        
        Ptr<QKDAppQoS> appBob = CreateObject<QKDAppQoS> ();  
        appBob->SetAttribute("AESLifetime", UintegerValue(aesLifetime));  
        appBob->SetAttribute("NumberOfKeyToFetchFromKMS", UintegerValue (numberOfKeyToFetchFromKMS));
        appBob->SetAttribute("AuthenticationType", UintegerValue (authenticationType)); 
        appBob->SetAttribute("EncryptionType", UintegerValue (encryptionType)); 
        ...
        // AMBIGUOUS CALL JAVÍTÁSA: Explicit kasztolás QKDApp014-re
        std::vector<std::string> connectionIDs = control->RegisterQKDApplications(
            DynamicCast<QKDApp014> (appAlice), 
            DynamicCast<QKDApp014> (appBob),
            kmsA,
            kmsB
        );
    ...
    // Ezek a Connect hívások továbbra is a QKDApp014 path-t használják, feltételezve,
    // hogy az örökölt jelek ezen az úton érhetők el.
